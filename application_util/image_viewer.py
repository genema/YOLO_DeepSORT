...
class ImageViewer(object):
    ...
    * 'ESC' : quit
    ...
    def point(self, x, y, w, h):
        xcent = int(x + .5*w)
        ycent = int(y + .5*h)
        pt1   = xcent-1, ycent-1
        pt2   = xcent+1, ycent+1
        cv2.rectangle(self.image, pt1, pt2, self._color, 1)
        np.save('tracks.npyfile, arr')

    def line(self, a1, a2):
        # Draw a line
        # Parameters a1[x1, y1], a2[x2, y2]

        pt1 = a1[0], a1[1]
        pt2 = a2[0], a2[1]
        cv2.line(self.image, pt1, pt2, self._color, self.thickness)
    ...
    def rectangle(self, x, y, w, h, label=None):
        ...
        if label is not None:
            ...
            center    = pt1[0] + 6, pt1[1] + 5 + text_size[0][1]
